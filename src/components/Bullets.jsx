...
export default function Bullets() {
    return (
        <nav className="flex items-center justify-center lg:pb-14" aria-label="Progress" >

            <ol role="list" className="ml-8 flex items-center space-x-5">
                {steps.map((step) => (
                    <li key={step.name}>
                        {step.status === 'complete' ? (
                            <a href={step.href} className="block h-2.5 w-2.5 rounded-full bg-green-600 hover:bg-indigo-900">
